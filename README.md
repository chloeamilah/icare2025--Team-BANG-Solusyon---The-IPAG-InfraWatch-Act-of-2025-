# The **IPAG–InfraWatch Act of 2025**

## Overview
This lowers corruption not by punishing more, but by making corruption harder, riskier, and easier to detect.

## Team Members
1. Jhanelle T. Sardoncillo
2. Chloe Amilah B. Cuaresma
3. Ashley M. Locsin 
4. Ashlei Jean Calugtong

## Policy Overview
The IPAG–InfraWatch Act of 2025 is a proposed Philippine law that establishes a centralized, tech‑powered, inter‑agency oversight system designed to detect, prevent, and prosecute corruption in infrastructure projects. It mandates real‑time digital monitoring, mandatory data transparency, and joint agency action between Commission on Audit (COA), Department of Public Works and Highways (DPWH), National Bureau of Investigation (NBI), Ombudsman, and civil society partners to ensure infrastructure spending is efficient, transparent, and corruption‑resistant.

## Technology Stack


## Methodology
Our approach followed a structured data pipeline:

### Data Collection
1. Excel dataset containing allotted budget per project from 2018 to 2025.
2. Covers multiple years across different presidential administrations.
3. Established a baseline for analyzing budget allocation trends over time.
4. We collected data from multiple reliable sources to ensure accuracy and comprehensiveness.

### Data Analysis
1. Tracked patterns in budget allocation across presidential terms.
2. Compared year‑to‑year changes to identify anomalies or shifts in priorities.
3. Highlighted correlations between leadership changes and spending trends.
   
### Visualization
Created charts to make findings accessible

### Validation
1. Cross‑checked results against official government reports and external references.
2. Verified consistency of trends with historical policy shifts.
3. Ensured reproducibility by documenting the workflow and versioning the dataset.

## Insights and Analysis


## Setup and Installation Guide

### SECTION 1 – Declaration of Policy
The State shall adopt a real‑time, technology‑driven oversight system to prevent corruption in public infrastructure by enforcing transparency, strengthening inter‑agency coordination, and ensuring that all government infrastructure projects comply with budgeting, procurement, and engineering standards.

### SECTION 2 – Creation of the National InfraWatch Platform
Real-Time Project Dashboard (Required Upload Every 72 Hours)
Agencies must upload:

* project name
* location (GPS)
* contractor & subcontractors
* bill of materials
* approved budget vs. actual disbursement
* geotagged progress photos
* timeline status
* change orders & reasons

Penalty for failure to upload data:
* ₱50,000 per missed update (official-in-charge)
* suspension after 3 violations

### SECTION 3 – Automated Red‑Flag Detection System (A‑RFD)
The system automatically scans for corruption indicators such as:
* 20%+ price variance from regional standard costs
* repeated winning of projects by one contractor
* unexplained cost escalation
* zero physical progress after 30 days
* suspicious subcontracting patterns
* mismatched materials vs. budget

When a red flag is detected:
The system automatically sends a case alert to:
*COA Special Audit
*NBI Anti‑Graft Division
*Office of the Ombudsman

They must respond within five (5) days.

### SECTION 4 – IPAG Inter‑Agency Oversight Council
A permanent council composed of:
* 1 COA Commissioner
* 1 DPWH Undersecretary
* 1 NBI Deputy Director
* 1 Ombudsman Representative
* 1 DILG Representative
* 1 licensed civil engineer from the private sector
* 1 civil society group representative

Mandate:
* Quarterly joint field inspections
* Approving red‑flag action plans
* Publishing public quarterly reports

### SECTION 5 – Contractor Integrity Score (CIS)
A scoring system from 0–100 based on:
* audit performance
* quality of past projects
* project delays
* blacklisting history
* safety compliance
* violation reports from communities

Rules:
* Contractors with CIS below 60 are barred from bidding.
* CIS of 40 and below → automatic investigation.

### SECTION 6 – Community Witness & Reporting Program (CWRP)
Citizens living within 5 km of a project can submit:
* poor-quality work
* slow progress
* missing materials
* ghost workers
via the InfraWatch mobile app.

Verified reports:
* ₱5,000 to ₱50,000 reward
* full legal protection
* anonymity preserved

### SECTION 7 – Digital Evidence Case Folder (DECF)
All InfraWatch data automatically forms a tamper-proof digital evidence file that includes:
* Timestamps
* uploaded documents
* photo metadata
* financial logs
* audit findings
This is directly transmitted to the Ombudsman and Sandiganbayan to speed up prosecution.

### SECTION 8 – Penalties
* Government officials who falsify data → 6–12 years imprisonment
* Contractors who fake progress reports → contract termination + 10‑year blacklist
* Agencies that delay uploads → administrative liability
* Tampering with digital evidence → treated as graft and corruption

### SECTION 9 – Funding
Funding from:
* General Appropriations Act
* 1% of DPWH annual budget dedicated to oversight
* fines collected from violations


## Data Sources Used

[Philippines – Country Profile – Transparency International](https://www.transparency.org/en/countries/philippines)

[PH Ranks 114th Among 180 Countries in 2024 Corruption Index – GMA News Online](https://www.gmanetwork.com/news/topstories/nation/958048/ph-ranks-114th-among-180-countries-in-2024-corruption-index/story/)

[Flood Control Fiasco: A Policy Reckoning for Accountability in the Philippines – Center for Integrative and Development Studies (UP CIDS)](https://cids.up.edu.ph/flood-control-fiasco-a-policy-reckoning-for-accountability-in-the-philippines/)

[Flood Control Corruption: An Obscene Plunder of Much-Needed Climate Funds – Greenpeace Philippines](https://www.greenpeace.org/philippines/press/68522/flood-control-corruption-an-obscene-plunder-of-much-needed-climate-funds/)

[Cracks in the Wall: How Flood Control Funds Sink into Corruption While Filipinos Drown – Reality of Aid](https://realityofaid.org/cracks-in-the-wall-how-flood-control-funds-sink-into-corruption-while-filipinos-drown/)

[Accountability Washed Away in Philippine Flood Control Corruption – East Asia Forum](https://eastasiaforum.org/2025/12/02/accountability-washed-away-in-philippine-flood-control-corruption/)

[Philippines’ Ranking Inches Up in Global Corruption Perceptions Index – Philippine Institute for Development Studies (PIDS)](https://www.pids.gov.ph/details/news/in-the-news/philippines-ranking-inches-up-in-global-corruption-perceptions-index)

[How Flood Control Projects Fail the Poor in the Philippines – PreventionWeb](https://www.preventionweb.net/news/how-flood-control-projects-fail-poor-philippines)

[E‑procurement and firm corruption to secure public contracts: The moderating role of governance institutions and supranational support (2022, Journal of Business Research)](https://www.sciencedirect.com/science/article/abs/pii/S0148296322005070)

[Detection of fraud in public procurement using data‑driven methods: a systematic mapping study (2025, EPJ Data Science)](https://link.springer.com/article/10.1140/epjds/s13688-025-00569-3)

[The Expediting Effect of Monitoring on Infrastructural Works (2021)](https://arxiv.org/abs/2102.09625)

[Global reviews of e‑procurement / open contracting (e.g. by the Open Contracting Partnership, NGOs and research)](https://www.open-contracting.org/anticorruption/)

[Recent reforms in the Philippines (New Government Procurement Act / NGPA, 2024)](https://www.philstar.com/opinion/2025/05/24/2445339/uncac-action-philippines-digital-reforms-advance-anti-corruption-commitments)

[Pilot / real‑world experiment: use of blockchain / digital tracking by Department of Public Works and Highways (DPWH)](https://www.philstar.com/the-freeman/opinion/2025/10/17/2480436/blockchain-technology-curb-corruption)
